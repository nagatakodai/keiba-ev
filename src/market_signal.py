...
def _interpret(ratio: float, n_horses: int) -> str:
    """place/win 比率を「型」に分類。
...
def _power_method_overround(probs: list[float], *, max_iter: int = 30, tol: float = 1e-6) -> list[float]:
    """raw 暗黙確率 (合計 > 1) を power-method で de-overround。

    Σ p_i^(1/k) = 1 となる k を bisection で解いて p_i^(1/k) を返す。
    ev.py:power_method_overround と同じロジック (簡易版)。
    """
    if not probs:
        return probs
    ...
    def f(k: float) -> float:
        try:
            return sum(p ** (1.0 / k) for p in items) - 1.0
    ...
    if f_lo == f_hi or f_lo * f_hi > 0:
        # bracket できなかったら overround をそのまま返す
        s = sum(items)
        return [p / s if p > 0 else 0.0 for p in probs]
...
def compute_market_signals(rd: RaceData) -> dict[int, MarketSignal]:
    ...
    place_normed = _power_method_overround(place_vals)
    # 複勝の合計は 3 にしたい (top 3 ポジション)
    s = sum(place_normed)
    if s > 0:
        place_normed = [v / s * 3.0 for v in place_normed]
    ...
    for h in horses:
        ...
        sig = MarketSignal(
            ...
            interpretation=_interpret(ratio, n) if wi > 0 and pi > 0 else "不明",
        )
        out[h.number] = sig
    return out
