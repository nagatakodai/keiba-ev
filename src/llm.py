...
def build_horse_score_prompt(
    ...
    *,
    ...
    lines = [
        ...
        "(0-100, 高いほど強い = 1着に近い)** を付けてください。これは市場とは独立した相対評価で、"
        "確率モデルの fundamental に合成され、最終的に市場オッズとブレンドされます。**買い目 "
        "(picks) は決めません** — あなたの仕事は各馬を相対評価して数値化することだけです。",
        "",
    ...
    lines += [
        ...
        "指数 (上げ・下げ問わず) を厚く採用する。材料が無ければ 0。",
        "",
        "## 出力",
        "全出走馬の 馬番→指数(0-100) と support を必ず網羅し、最後に以下の JSON を ```json ... ``` で出力:",
        "```json",
...
def parse_horse_scores(text: str) -> dict:
    ...
    scale="strength" を付け、ev 側は softmax(指数/T_LLM) の温度パスで確率化する。後方互換で
    `win_prob` (推定勝率 %) も拾い、その場合 scale="prob" (温度なし直接使用)。
    """
    raw = parse_evidence(text)
    if not isinstance(raw, dict):
        return {"scores": {}, "support": {}, "scale": "strength", "notes": {}, "summary": "", "confidence": ""}
    src = raw.get("scores")
    scale = "strength"
