...
def build_horse_score_prompt(
    ...
    *,
    ...
    lines = [
        ...
        "  - **パドック評価・馬体・返し馬** (← 締切~5分前にまさに実施中の最重要直前情報。落ち着き/"
        "イレ込み/発汗/歩様/毛艶/トモの張り・気配上昇下降。各馬これは **専用クエリで必ず確認** する)",
    ...
    lines += [
        ...
        "クエリ) まで** Tavily で補強してよい。**締切~5分前=パドック実施中なので、各馬まず "
        ...
        "**クエリ例 (①②を狙う)**: \"<馬名>\" パドック OR 気配 OR 歩様 OR 馬体重 <YYYYMMDD> (← パドック専用・各馬必須) / "
        "\"<馬名>\" 取消 OR 除外 OR 馬体重 / \"<場名>\" <YYYYMMDD> 馬場 含水率 / "
        ...
        "- **調整幅は原則 仮指数 ±20 以内**。仮指数から ±20 を超えて動かすのは、①取消/除外(→0) か "
        "②決定的な直前情報の複合 (例: 大幅馬体重増減 +12kg超 と パドックで明確なイレ込み/歩様難が重なる、"
        "主戦→大幅弱化の乗替り 等) がある時に限る。**根拠1つで極端に動かさない** (= 出走表を大きく覆すには"
        "相応の直前情報が要る・楽観バイアスと暴走の抑制)。evidence にその調整理由を必ず書く。",
        "",
