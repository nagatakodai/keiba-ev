...
def main(
    ...
    (llm_index, llm_support, llm_scale, llm_scored_at,
     llm_alerts, llm_evidence, llm_paddock) = _load_llm_scores(race_id)
...
def _save_llm_scores(race_id: str, parsed: dict, *, model: str) -> None:
    ...
    payload = {
        ...
        # paddock: 各馬のパドック評価 {"rating","note"} (締切~5分前の直前情報)。記録/表示用。
        "paddock": {str(k): dict(v) for k, v in (parsed.get("paddock") or {}).items()},
...
def _load_llm_scores(race_id: str, *, max_age_sec: int = 1800):
    """`<race_id>.llm.json` を読み (scores, support, scale, scored_at, alerts, evidence, paddock) を返す。
    ...
    evidence=dict[int,list[str]] (補強根拠の詳細配列、表示用)、paddock=dict[int,{"rating","note"}]
    (パドック評価、表示用)。
    無い / 壊れている / 古すぎる (max_age_sec 超過) なら (None,None,"strength",scored_at,None,None,None)
    を返し、bet ステージはモデルのみにフォールバックする。alerts/evidence/paddock は確率には使わず snapshot 表示用。
    ...
    if not p.exists():
        return None, None, "strength", None, None, None, None
    try:
    ...
    if scored_at:
        try:
            ...
            if age > max_age_sec:
                return None, None, "strength", scored_at, None, None, None
        except ValueError:
            pass
    ...
    paddock = llm_mod._normalize_paddock(d.get("paddock"))
    return ((scores or None), (support or None), scale, scored_at,
            (alerts or None), (evidence or None), (paddock or None))
...
def _run_score_stage(
    ...
    if not (os.environ.get("KEIBA_SCORE_FORCE_RESCORE") or "").strip():
        c_scores, c_support, c_scale, c_at, c_alerts, c_evidence, c_paddock = _load_llm_scores(
            race_id, max_age_sec=10**9)
        if c_scores:
...
def _build_index_compare(
    ...
    for n in nums:
        ...
        pd_n = paddock.get(n) if isinstance(paddock.get(n), dict) else None
        rows_out.append({
            "number": n,
            "name": names.get(n, ""),
            ...
            "evidence": ev_n,
            # パドック評価 (締切~5分前の直前情報)。無い馬は null。
            "paddock": ({"rating": str(pd_n.get("rating", "")), "note": str(pd_n.get("note", ""))}
                        if pd_n and (pd_n.get("rating") or pd_n.get("note")) else None),
        })
    rows_out.sort(
        key=lambda r: (
...
def _save_prediction_snapshot(
    ...
    snapshot = {
        ...
        # 各馬のパドック評価 {"rating","note"} (締切~5分前の直前情報)。記録/表示用 (確率には未使用)。
        "llm_paddock": ({str(k): dict(v) for k, v in llm_paddock.items()}
                        if llm_paddock else None),
        "llm_scale": llm_scale,
        "llm_blend": llm_blend,
...
def _refresh_and_reevaluate(
    *,
    ...
    (llm_index2, llm_support2, llm_scale2, llm_scored_at2,
     llm_alerts2, llm_evidence2, llm_paddock2) = _load_llm_scores(race_id)
    ...
    if not no_cache:
        _save_prediction_snapshot(
            ...
            llm_evidence=llm_evidence2, llm_paddock=llm_paddock2,
