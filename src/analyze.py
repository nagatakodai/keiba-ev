...
def main(
    ...
    if html_file:
        ...
        rd = fetch_and_parse(url)  # type: ignore[arg-type]
    ...
    if not no_cache:
        _save_prediction_snapshot(race_id, rd, rows, plan_rows)
    ...
    _print_plans(plan_rows, hit_points=hit_points, hit_budget_ratio=hit_budget_ratio)
    ...
    if not no_llm:
        ...
        if evidence:
            ...
            if not no_cache:
                _save_evidence_to_snapshot(race_id, plan_rows, evidence)
...
def _print_evidence_adjusted(
    ...
    if adjusted:
        ...
        console.print(tbl)
    _print_plans(adjusted, hit_points=hit_points, hit_budget_ratio=hit_budget_ratio)


def _save_evidence_to_snapshot(race_id: str, plan_rows, evidence: dict) -> None:
    ...
    plan_f = ev_mod.plan_final(plan_a, plan_b, plan_c, plan_h1, plan_h2)
...
def _save_prediction_snapshot(race_id: str, rd, rows, plan_rows) -> None:
    plan_a = ev_mod.plan_balanced(plan_rows)
    plan_b = ev_mod.plan_max_ev(plan_rows)
    plan_c = ev_mod.plan_wide(plan_rows)
    plan_h1 = ev_mod.plan_hit_pure(rows, target=3)
    plan_h2 = ev_mod.plan_hit_safe(rows, target=3)
    plan_f = ev_mod.plan_final(plan_a, plan_b, plan_c, plan_h1, plan_h2)
    snapshot = {
        "race_id": race_id,
...
def _print_plans(
    ...
    console.print(
        f"[dim]予算配分: +EV 枠 ¥{ev_budget:,} (Plan A/B/C のいずれか) / "
...
def _refresh_and_reevaluate(
    *,
    ...
    if not no_cache:
        _save_prediction_snapshot(race_id, rd2, rows2, plan_rows2)
    ...
    _print_plans(plan_rows2, hit_points=hit_points, hit_budget_ratio=hit_budget_ratio)
