...
# 【較正 2026-07-05 (scripts/sweep_llm_blend.py, n=131 市場非依存 era・発走前採点・結果確定)】
#   勝者 log-loss の (T, w) grid sweep で **旧値 (T=25, w=0.75) は市場単独より一貫して悪い**
#   (log-loss 1.909 vs 市場 1.810 = +0.10。前半/後半・v1/v2/v3 の全分割で +0.08〜+0.12 と再現)。
#   これは EV束の実測 ROI 51.5% のメカニズムそのもの: w が大きいほど最終Pが Claude の言い分で
#   市場から乖離し、その乖離が勝率次元で情報を持たない (むしろ逆) ため乖離した所だけ買って
#   負ける (adverse selection)。最小は (T=10, w=0.25) の 1.801 だが市場単独比 -0.009 は
#   分割で反転する (前半 -0.055 / 後半 +0.036) ノイズ域 → **確率合成での Claude 付加価値は
#   現状ゼロが誠実な結論**。w=0 (完全除去) も統計的に同等だが、ユーザ決定 (2026-06-13) の
#   Claude 反映を残しつつ出血を止める妥協点として最小セル (10, 0.25) を採用。
#   ※ Claude 指数の**順位**ベースの価値 (shobu 戦略くらべ・基準B・プレレジ台帳) はこれとは
#   独立に測っており、そちらは市場人気基準線に対し正の傾向 — 「順位には価値の兆候・確率の
#   絶対値には無い」が現状の実測。指数付きレース +100R 蓄積ごとに再 sweep すること。
LLM_BLEND_DEFAULT = 0.25   # 旧 0.75 (2026-06-13〜2026-07-05)
MARKET_BLEND_LIVE = BLEND_DEFAULT
T_LLM = 10.0               # 旧 25.0 (同上)
