...
def _dump_betform(page) -> None:
    """賭式チェックボックスと 裏目/マルチ の DOM 構造を JSON で吐く (セレクタ確定用)。"""
    info = page.evaluate(
        "() => { const out=[]; "
        ...
        "  const txt = (lbl||'').replace(/\\s+/g,'');"
        "  if (/betTypeSelect|\\u88cf\\u76ee|\\u30de\\u30eb\\u30c1/.test(cb.name+'|'+txt)) {"
        "    out.push({name:cb.name, id:cb.id, value:cb.value, checked:cb.checked,"
        "              label:txt.slice(0,24),"
        "              parentId:(cb.parentElement?cb.parentElement.id:''),"
        "              parentClass:(cb.parentElement?cb.parentElement.className:'')}); }"
        "} return out; }")
    print("  賭式/裏目/マルチ checkbox 一覧:", flush=True)
    for c in info:
...
def main() -> None:
    ...
    if do_set:
        print("\n===== pass2: 馬単1脚 試投入 (購入はしない) =====", flush=True)
