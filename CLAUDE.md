...
- **Claude 指数 単複戦略の仮想収支 (2026-06-30 ユーザ指示)**: 「Claude 指数1位の単勝を買った / 2,3位の複勝を買った と仮定した計測も過去分全て」。BOX とは別の戦略カードとして、各 shobu 評価レースで **Claude 指数1位の単勝 + 2・3位の複勝を各 ¥point_cost** 買ったと仮定し、`data/results/<id>.json` の `final_odds` (`win:N`/`place:N` = ×100 オッズ、例 `win:7=19.2`→¥1920) で払戻を集計 (`api/store.py:_winplace_race_pnl`/`_winplace_pnl`、`compute_shobu_winplace_pnl`=推奨 / `compute_indexed_winplace_pnl`=shobu 評価全体)。**複勝は頭数ルール** (`_place_cutoff`: 4頭以下=発売なし / 5-7頭=2着まで / 8頭以上=3着まで) を適用。母集団は BOX と同じ `_shobu_eval_races(recommended_only=)` を共有 (BOX 集計から抽出した共通ヘルパ)。払戻オッズは **的中脚のみ要求** (外れ脚は¥0なのでオッズ欠落でも成立)、的中脚のオッズが無い (final_odds 未保存=keiba.go.jp fallback 等) レースのみ `no_odds` で分母外。返り値は全体 ROI/net + **単勝・複勝それぞれの bets/hits/hit_rate/ROI を分離**。`GET /api/shobu/winplace-pnl` / `/api/shobu/indexed-winplace-pnl`、`api.winplacePnl()`/`indexedWinplacePnl()`、ダッシュボードは `WinPlacePnlCard` で併記。**初回実測 (2026-06-30, 全評価 68R)**: 単複合計 ROI ~85% (単勝#1 的中35%・ROI 86% / 複勝#2,3 的中43%・ROI 85%) = 控除 (単勝20%/複勝20%) を僅かに上回る程度。`tests/test_shobu_pnl.py::test_winplace_*`。
