...
- **全 Claude 指数レースの仮想収支を別カードで併記 (2026-06-28 ユーザ指示)**: 「Claude 指数が**全ての馬についていて**結果があればダッシュボードに反映する」。勝負レース(推奨)収支とは**別カード (非破壊)** で、**recommended に限らず全出走馬に Claude 指数が付いて結果が確定した全レース**の上位N頭3連単BOX 仮想収支を表示 (`GET /api/shobu/indexed-pnl`, `api/store.py:compute_indexed_pnl`)。対象 = `data/predictions` で `len(Claude指数) ≥ n_runners` (全馬充足) かつ結果あり (実測 ~152 レース)。BOX/的中/配当ロジックは勝負レース収支と共通の `_box_race_pnl(..., require_full_index=True)` に切り出し (両者で重複なし)。母集団が広いぶん回収率は控除水準 (~72.5%) へ収束しやすい全数 paper 指標 (推奨収支=screen の強いレースのみ、こちらは全 Claude 解析レースの素の成績)。ダッシュボード (`web/app/page.tsx`) は hero (推奨) の下に `IndexedPnlCard` を併記。`api.indexedPnl()`。
