...
- **option (CLI / Web UI 両対応)**: しきい値 (edge_threshold / edge_margin)・対象 (all/jra/nar=地方平地/**banei=帯広ばんえい**, `ev.segment_of_rd` と同じ3区分で分離)・発走前のみ・Claude 一括生成 (claude_all) / 発走が近い順 N件 (claude_eval)。`shobu_score` = 基準B のスコアそのもの。UI は推奨を専用「勝負レース(推奨)」セクション (緑カード+番号) に分離し、推奨外は折りたたみ。
