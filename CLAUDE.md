...
### bet-type-specific market_blend (Phase 19, holdout 291 races 由来)

`src/eval_holdout.py` の real-odds 評価で β の最適値が bet type ごとに違うことが判明。Plan ごとに β を使い分ける:

| Plan / bet type | β | 根拠 |
|---|---|---|
| Plan A/B/C/G/H2、単勝 EV table、3 連単 EV table | **0.78** (`BLEND_DEFAULT`) | 単勝 ROI peak 95.9% (市場 88.5% +7.4pt)、3 連単 PL hit rate も β=0.7-0.8 plateau |
| Plan H1 (確率最優先 3 点) | **0.0** (`BLEND_HIT_PURE`) | holdout で唯一 +EV を達成 (ROI 109.6%, hit 15/291)。pure LightGBM が市場と独立した outsider 信号を出している |

`src/analyze.py` は同じ `rd` に対して `estimate_probs` を 2 回呼び、`plan_rows` (default) と `plan_rows_hit` (β=0) を作って Plan H1 だけ後者を使う。Plan A/B/C/G/H2 は前者のまま。
