...
- 運用: 端末A `python -m src.oddspark_bet --session` (ログインして放置) / 端末B `make watch-auto` 相当を `--bet-oddspark` 付きで起動 → 発走前 NAR の束が常駐ブラウザのカートに積まれ続け、人が目視で確定。`BettingSession` は one-shot `fill_cart` とも共用 (one-shot は1レース→input待ち→close、daemon は queue ループ)。
