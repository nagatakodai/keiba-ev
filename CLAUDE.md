...
- **自動取得 (`ShobuAutoManager`, make api 稼働中, 2026-06-20 ユーザ指示)**: API に常駐 asyncio ループを持たせ、既定 **5 分毎** (`interval_sec`) に勝負レース スキャンを Job 経由で自動再実行 → `data/cache/shobu/<date>.json` を上書き。frontend は 30 秒毎に result + auto status を polling して最新化 (ボタンを押さなくても結果が更新され続ける)。設定 (enabled/interval/options) は `data/cache/shobu_auto_state.json` に永続化し uvicorn 再起動後も復元。`GET/POST /api/shobu/auto` (UI のトグル「自動取得」+ 間隔入力) で制御。**claude_all 既定 ON** なので各 tick で未スコア発走前レースに claude -p を spawn する (スコア済は二重生成しないので定常状態は安い・初回 tick だけ重い)。interval/enabled 変更は `_wakeup` (asyncio.Event) で in-flight の待機を起こして即時反映。lifespan startup で起動・shutdown で停止。
