...
### bet-type-specific market_blend (Phase 19/20/21, holdout 291 races + T=0.4 由来)

`src/eval_holdout.py` の real-odds 評価で β の最適値が bet type ごとに違うことが判明。Plan ごとに β を使い分ける:

| Plan / bet type | β | holdout 結果 (T=0.4) |
|---|---|---|
| Plan A/B/C、単勝 EV table、3 連単 EV table | **0.78** (`BLEND_DEFAULT`) | 単勝 ROI peak 95.9% (市場 88.5% +7.4pt) |
| Plan G (適性ゲート → EV 足切り) | **1.0** (`BLEND_APTITUDE_GATE`) | **ROI 108%, hit 5/291** (β=0→1 単調) ★ |
| Plan H1 (確率最優先 3 点) | **0.0** (`BLEND_HIT_PURE`) | **ROI 101%, hit 12/291** ★ |
| Plan H2 (確率 + P×O≥1.0) | **0.0** (`BLEND_HIT_PURE`) | **ROI 132%, hit 11/291** ★★ |

`src/analyze.py` は同じ `rd` に対して `estimate_probs` を 3 回呼び、`plan_rows` (default β=0.78) / `plan_rows_hit` (β=0) / `plan_rows_apt` (β=1.0) を作って各 Plan で使い分ける:
- Plan A/B/C: `plan_rows`
- Plan G: `plan_rows_apt`
- Plan H1/H2: `plan_rows_hit`

### Plan の実証的な階層 (holdout 291 races)

| Plan | 評価 | ROI | hit/291 | コメント |
|---|---|---|---|---|
| **G** | ★★ +EV | 108% | 5 | 適性 × 市場の PL 連鎖だけで +EV、小サンプル |
| **H1** | ★ +EV | 101% | 12 | pure LGBM の確率上位、平均オッズ高 |
| **H2** | ★★★ +EV | 132% | 11 | LGBM 上位 + EV ≥1 フィルタ、Phase 21 で最も安定 |
| A | -EV / noise | 54% | 1 | hit が小さく結論できない |
| B | -EV | 0% | 0 | 楽観バイアスの罠 (Plan B の節参照) |
| C | -EV | 57% | 2-3 | hit が小さく結論できない |

長期黒字を狙うなら **Plan G / H1 / H2** が現状の証拠で支持される。Plan A/B/C は N=291 では証拠が弱く、利用するなら検索 MCP の補強根拠で慎重にフィルタすること。
