...
- **プレレジ (事前登録) シグナルルール + walk-forward ガードレール (2026-07-05 ユーザ指示「研究中シグナルを用いて回収率を格段に上げる」)**: 研究中シグナルを行動に変える正攻法として **「発見と検証の分離」** を実装。walk-forward バックテスト (105R, look-ahead なし・レース t 以前の履歴だけでマトリクス best セルを選ぶ) で **セル追従は機能しない** ことを実測 (追従 ROI 48-66% < 馬連固定 78%・「確証セルのみ」は一度も発火せず・in-sample 主役の 馬連×一致×本命×NAR 202% は v2 時代 36% に崩壊 = trio1234box 失効と同型の bin-selection)。よってセル自動追従は実装せず、**ルールを定義凍結 (プレレジ) → 登録日以降のレースのみで ROI+bootstrap CI を蓄積 → 確証★ (n≥`SIGNAL_RULE_MIN_CONFIRM`=30 かつ CI下限>1.0) / 破綻 (n≥20 かつ CI上限<1.0) を自動判定** する台帳を作った。配線: `api/store.py` の `SIGNAL_RULES` (**定義変更禁止** — 変えるなら新 key で再プレレジ) + `_tagged_eval_races` (compute_market_agreement と共有の条件タグ付きレコードローダ・リファクタ出力同値確認済) + `compute_signal_rules` (insample=参考 / prospective=確証判定 / market_baseline=同条件で市場人気順に同じ買い方 / dead_cell / walkforward) + `append_signal_rules_history` (`data/cache/signal_rules_history.jsonl`, ResultAutoFetcher が毎 tick 追記・races 不変は no-op) + `GET /api/shobu/signal-rules`。UI: ダッシュボード `SignalRulesCard` (マトリクスカードの上・発見時/登録後/市場人気基準/状態 の表 + walk-forward ガードレール注記) と 予測詳細の「プレレジ」バナー (`betGuide.raceSignalRuleGuide` が per-race で発火ルール判定・死にセル=拮抗×不一致 は**見送りゾーン**表示・破綻ルールは非表示。条件定数 `MARKET_INDEX_T`/`FAVORITE_RATIO_THRESHOLD`/JRA 10場 は backend のミラー)。**登録ルール 6 本 (2026-07-05, 発見時 105R 実測)**: ①place2_bigfield (複勝2×≥12頭, 123%・drop-best 105% = 唯一 drop-best 後も 100% 超) ②place2_bigfield_agree (×一致, 162%/126%) ③place1_consensus (複勝1×一致, 97%。市場#1複勝全体 80%・Claude 不一致側の人気馬複勝 74% に対し**一致選別の付加価値が実在** — なお一致条件下は Claude#1=市場#1 で同じ馬を買うため market_baseline 列とは定義上同値になるのが正しい) ④win1_smallfield (単勝1×≤8頭, 123% だが drop-best 92%=単発依存疑い) ⑤quinella12_alive (馬連×死にセル回避, 97% vs 死にセル 47% — 見送り規律) ⑥quinella12_agree_honmei_nar (202% セルの追試・崩壊検証用)。固定戦略ベースラインは全て <100% (馬連 78% だが前半135→後半22 と不安定)。**確証★が出るまでは全ルール参考表示・実弾には接続しない** (確証後も接続は人の判断)。
  ...
  - **券種比較グリッド 30 本を一括プレレジ + 新戦略 3 種 (2026-07-05 ユーザ指示「上から単勝1,単勝2,単勝3,複勝1,複勝2,複勝3,馬連1-2,馬連1-3,ワイド1-2,ワイド1-3 で並べる。条件の定義はそれぞれ固定感のあるものにして比較する」)**: `SIGNAL_RULES` に **10 券種 × 固定 3 条件 (無条件 / 市場一致 / 荒れ模様 fav_odds≥2.5)** の `grid_<戦略>_<条件>` ルールを自動生成で追加 (`_GRID_STRATEGIES`/`_GRID_CONDITIONS`)。条件を全券種で統一したことで「どの券種がどの状況で強いか」を同じ土俵で比較できる。このために **win2/win3 (指数2/3位の単勝)・quinella13 (指数1-3位の馬連) を STRATEGY_DEFS へ追加** (`_strategy_race_legs` に脚・STRATEGY_SHORT_LABELS にラベル。戦略くらべカード/的中バッジにも自動で載る)。STRATEGY_DEFS の表示順もユーザ指示の順 (単勝1,2,3→複勝1,2,3→馬連1-2,1-3→ワイド1-2,1-3→その他) に並べ替え。`compute_signal_rules` は rules をこの券種順 (グループ内は grid→個別発見ルール) にソートして返す (計 40 本, ~1.1s/計算)。**注意**: 新戦略の的中脚が increase する分、result final_odds に無い組の的中で no_odds になり得るが、実データは in-money 全券種保存 (2026-07-04〜) のため 108R で減少ゼロ (テスト fixture は要追記 — 同日修正済)。グリッド in-sample の初見: 一致×馬連1-2 120% / 荒れ×馬連1-3 135% / 荒れ×単勝1 119% が上位、荒れ×馬連1-2 48% / 荒れ×ワイド1-2 31% が下位 (すべて蓄積中・確証なし)。
    - **馬連1-2+1-3 両方買い 3 本を追加プレレジ (2026-07-06 ユーザ指示「馬連1-2,1-3を両方買った場合の回収率も研究中シグナルに追加」)**: 新戦略 **quinella12_13** (指数1-2位馬連 + 1-3位馬連の2脚/レース・的中は排反なので高々1脚・¥200/R) を STRATEGY_DEFS (quinella13 の直後)・`_strategy_race_legs` (既存 quin_leg+quin13_leg の合算 = ≤1.1 見送りフィルタは脚単位で継承)・STRATEGY_SHORT_LABELS へ追加し、グリッドと同じ固定3条件で `grid_quinella12_13_{all,agree,rough}` を **registered_at 2026-07-06** でプレレジ (既存グリッド 07-05 の凍結は不変・計45本)。in-sample 初見 (113R): 無条件 76% (市場基準78%) / 一致 92% (85%) / **荒れ 92% (市場基準41%)** — 荒れ条件で市場人気順との差が大きいのが観察点 (蓄積中・確証なし)。`tests/test_shobu_pnl.py::test_signal_rules_quinella12_13_prereg`。
    - **単勝1+複勝3 両方買い 3 本を追加プレレジ (2026-07-06 ユーザ指示「荒れ×(単勝1+複勝3) も研究しよう」)**: **ペア併用スイープ** (105ペア×3条件=315セルの読み取り専用探索、同日) の結論 — ①**単純併用の ROI は必ず構成戦略の加重平均** (最良単独を超えるペアは 0/105 = 原理どおり) ②上がり得るのは **CI下限 (分散低減) と条件別の使い分け**。CI下限が全組み合わせ中最高だったのが **荒れ×(単勝1+複勝3)** (発見時 111R: 108% CI[72,151]・単独 CI下限 60/55 に +12pt)。新戦略 **win1_place3** (2脚/レース・複勝発売なし少頭数は単勝1のみ・place3 直後) を追加し `grid_win1_place3_{all,agree,rough}` を registered_at 2026-07-06 でプレレジ (計48本)。**⚠ 315セルからの事後選択 = 多重比較バイアス大なので prospective 判定が本命** (walk-forward のセル追従破綻と同型のリスクを明示して登録)。条件別使い分け (荒れ→X, 一致→Y) はセル追従そのものなので実装しない。`tests/test_shobu_pnl.py::test_signal_rules_win1_place3_prereg` / `::test_strategy_win1_place3_win_only_when_no_place`。
    - **Claude 指数の絶対値 (idx1/2/3) 特徴量 + ルール 2 本を追加プレレジ (2026-07-06 ユーザ指示「Claude指数1,2,3の値がどれくらいだったか の条件も計測したい (指数60と70で指数1位なのは差があるはず)」)**: `_race_features` に **idx1/idx2/idx3** (Claude 指数 1/2/3位の生値。0-100 レース内相対だが v3 は仮指数アンカーが出走表由来なのでレース間でも水準に意味を持ち得る) を追加 (実分布 111R: idx1 p25=72/med=78/p75=83・sd 9.6 と広がり十分)、betGuide.raceSignalFeatures にも同ミラー。sweep (閾値は四分位近傍の丸値) から ⑮**win2_top1confident** (単勝2×idx1≥80, 114%・n=47・**drop-best 100.2%=単発抜きでも100超**・前半107/後半119 と安定・市場基準67% — idx系で n と安定性最良) ⑯**trio1234box_top1confident** (3連複BOX×idx1≥85, 148%/drop103%/前半175後半122/市場基準37%・n=25。**trio1234box は無条件で失効歴のある戦略 — 高確信条件で蘇るかの追試枠**) を登録 (registered_at 2026-07-06, 計50本)。低idx側 (idx1≤70 等) は的中率1桁%・CI[0,300+] のノイズ域で登録なし。`tests/test_shobu_pnl.py::test_signal_rules_idx_confidence_prereg`。
  - **FL バイアス (単勝/複勝オッズ比) 特徴量 + ルール 2 本を追加プレレジ (2026-07-06, 「他に回収率を上げられることは」の続き)**: `_race_features(idx, mkt, snap)` に **pw_top1/2/3** (Claude 上位1-3位馬の 単勝オッズ÷複勝オッズ、snap の bet_tables 実オッズ。高い=市場が「絡むが勝ち切らない」3着型と評価。複勝はレンジ下限なので比は系統的にやや高め=レース間で一貫) を追加。sweep (108R) から ⑪**quinella13_top2winner** (馬連1-3×pw_top2≤3.0=2位が勝ち切り型, 141%・前後半 141/142 と安定・市場基準52%・ただし drop-best 92%=単発寄与大) ⑫**place2_top2placer** (複勝2×pw_top2≥4.0=2位が3着型, 102%/drop90%・市場基準64%との差が主眼) を登録 (registered_at 2026-07-06)。frontend は `raceSignalFeatures(items, betTables)` — 予測詳細ページが `d.bet_tables` を IndexCompareCard→raceSignalRuleGuide に渡して発火判定 (bet_tables 欠落は None=不発火の保守側)。`tests/test_shobu_pnl.py::test_race_features_pw_ratio_from_bet_tables`。
  ...
  - **対市場基準Δ + レース単位ペア bootstrap CI 列 (2026-07-06, 回収率コンサル実行)**: ★判定 (ROI CI下限>100%) だけでは place1_consensus 型の「market_baseline と定義上同値」ルールが付加価値ゼロでも★になり得るため、**「市場人気順に同じ買い方をした基準線に勝っているか」を判定補助列として常設** (★ロジック自体は不変更)。`api/store.py:_paired_delta_ci` が同一発火レース集合上で rule と baseline の (Σstake,Σpayout) を**レース単位ペアのまま再標本化** (脚単位 iid は 2026-07-04 の過小CIバグと同型なので禁止) し、各ルールに `insample_vs_baseline` / `prospective_vs_baseline` = `{delta (ROI差, 0.45=+45pt), ci_low, ci_high, beats_baseline (=ci_low>0), n}` を付与 (ペア母集団 <5R は null)。history compact 行にも prospective 側 `vs_baseline` を追記。UI: SignalRulesCard に「対市場Δ (登録後)」列 (beats=緑・0跨ぎ=淡色・null=—)。実測 (108R insample): place2_bigfield **Δ+69pt CI[+6,+139] beats=True** / wide13_top3consensus Δ+65pt CI[+2,+138] beats=True / place1_consensus Δ=0 CI[0,0] (同値定義の恒等0が正しく出る・テスト固定)。実弾接続プロトコル (下記) の接続条件 (a) がこの列を参照する。`tests/test_shobu_pnl.py` (ペアのレース単位性/恒等0/e2e スキーマ)。
  - **確証後の実弾接続プロトコル (凍結 2026-07-06, 回収率コンサルの帰結)**: 確証★が出てから配分を考えると裁量の bin-selection が混入するため、**接続条件と staking を事前凍結**する。変更するなら本節を書き換えた commit を残すこと (プレレジと同じ規律)。①**接続条件**: 確証★ (prospective n≥30 かつ ROI CI下限>100%) に加え、(a) **market_baseline に対する Δ の CI 下限 > 0** (市場人気順に同じ買い方をする基準線に有意に勝つこと — place1_consensus 型の「基準線と同値」ルールは付加価値の証明にならない) (b) family-wise 偽★対策として **★確定後さらに +30〜50R の hold-out 追試で ROI>100% を維持** (42本同時監視の一時的偽★率は~10%/ルール規模) (c) 接続は人の判断 (自動接続しない)。②**staking**: 確証ルール発火レースのみ・**1/4 Kelly 相当を上限** (edge 推定は prospective ROI の CI **下限**を使う=楽観禁止)・複勝/単勝など低控除券種を優先。③**NAR pool インパクト上限**: 投票額 ≤ 当該 pool の ~1% 目安 (odds_timeline 実測で NAR は自票で 44% の脚が margin 1.10 を食い破る下振れ — NAR 確証はスケールしない前提、スケールは pool の深い JRA のみ)。④**daily cap / per-race 上限は既存の投票 daemon 安全ゲートを流用**し、接続後も rolling ROI が CI 下限を割ったら即停止 (破綻判定 n≥20 CI上限<100% を待たない)。
- **前日夜間の翌日一括解析 (`api.main.ShobuNightlyPrescanner`, make api 常駐, ユーザ指示 2026-07-05「前日夜間に次の日の勝負レースを全て解析できるようにしたい」)**: 毎晩 **`KEIBA_NIGHTLY_SCAN_HOUR` (JST, 既定21時・12-23クランプ) 以降の最初の tick** で **翌日** の shobu スキャン (race_type=all・claude_all=True) を Job として1回だけ起動 (`KEIBA_NIGHTLY_SCAN=0` で無効・`KEIBA_NIGHTLY_RESEARCH=prefetch` で ARCH-B 切替・実行済みガードは `data/cache/shobu/nightly_state.json` に永続 = reload 二重スキャン防止)。前夜に成立する根拠 (実機 2026-07-05 夕): ①discovery は 3 ソースとも日付引数対応で翌日 36R (帯広/盛岡=oddspark・川崎=keibago) を列挙 ②`find_keibago_race` は rid 由来日付で翌日を解決し DebaTable (出馬表) も前夜取得可 ③オッズ未発売レースは odds_empty 早期 return で**指数のみキャッシュ** (2026-06-24 機構) → 当日の再スキャン/refresh が市場を付けて基準Bを確定 (指数は再利用=当日コストほぼゼロ)。**Web UI からも手動可**: 勝負レース設定パネル「対象日」Select (今日/翌日) → `ShobuScanRequest.date` (結果ポーリング/refresh は `job.date`/`result.date` で翌日ファイルを追従)。状態 `GET /api/shobu/nightly`。付随修正2件: ①**shobu の NAR score subprocess に oddspark フォールバック** (`_run_claude_eval._one` を `_exec` 抽出・keibago が rc≠0 (timeout以外) なら `src.scrape_oddspark --phase=score` を1回試す — keiba.go.jp に翌日カード未掲載の場 (実測: 盛岡) を救う。scrape_oddspark にも `--model=` 追加) ②**パドック再score の no-op 実バグ修正**: `ShobuPaddockRescorer._rescore` が `KEIBA_SCORE_FORCE_RESCORE=1` を立てておらず、指数再利用 (2026-06-24, max_age=∞) に当たって**再検索が一切走っていなかった** — 前夜生成指数を当日直前に更新する経路はここしか無いので致命的だった。`tests/test_shobu_auto.py::test_nightly_prescanner_*`。
- **当日キャッチアップ自動スキャン (`api.main.ShobuDailyCatchupScanner`, make api 常駐, 2026-07-06 回収率コンサル実行)**: 台帳蓄積が ~9R/日 (108R/15日・JRA はわずか 9R) しかなくプレレジ確証のボトルネックだった主因 = **当日スキャンが手動ボタン依存** (nightly は「翌日」を21時に1回だけ・make api が夜間落ちていれば当日データ無し)。対策として **毎日 `KEIBA_DAILY_SCAN_HOUR` (JST, 既定9時・6-15クランプ) 以降、当日スキャン (race_type=all・claude_all=True) を自動起動**。**⚠ 初日 (2026-07-06 09:02) の実機で「1日1回」設計の事故を確認 → オッズ発売までリトライする方式に同日修正**: NAR オッズ発売前 (盛岡1R 11:50 発走に対し 09:28 時点でも単勝 0 組) にスキャンすると全 score subprocess が odds_empty 早期 return (snapshot 無し・数秒で終了)、`.llm.json` は前夜 nightly 分が在るので `_has_index` が True → **36/36「生成成功」・推奨 0 件・snapshot 0 で正常終了**し、以後の常駐ループはどれも対象にしない (2分毎 refresh / パドック再score は **推奨レースのみ**) ため**当日の台帳が空のまま確定**する。修正後の発火条件: 当日ファイルの **snapshot 被覆 (`summary.with_snapshot/evaluated`) が `DONE_SNAPSHOT_RATIO` (0.9 — 0.5 だと昼+夕方の2場が付いた 24/36=67% で打ち切られ、発売が最も遅い帯広ばんえいが永久に載らない) に達するまで、`KEIBA_DAILY_SCAN_RETRY_MIN` (既定60分・15-240クランプ) 間隔で最大 `KEIBA_DAILY_SCAN_MAX_ATTEMPTS` (既定8回・attempts は `daily_scan_state.json` に永続、旧形式は attempts=1 として読む) 回**。間隔判定は当日ファイル mtime 基準 (手動スキャン直後は自然に待つ)・前回 Job 実行中は発火しない・オッズ発売後の最初のリトライで snapshot が付き自動完了する (指数は再利用なので各リトライの Claude コストほぼゼロ、オッズ発売前のリトライは全レース数秒で終わる)。`KEIBA_DAILY_SCAN=0` で無効・research 既定 "ab" (`KEIBA_DAILY_RESEARCH`)。状態 `GET /api/shobu/daily` (retry_min/max_attempts/attempts_today)。**「夜間 nightly 後に予測履歴が空」は正常** (オッズ未発売 → odds_empty 早期 return で snapshot 無し = 履歴一覧に出ない。`.llm.json` 指数は生成済で、当日オッズ発売後の daily リトライが snapshot を付けて履歴に載せる)。
  - **付随修正: 取消二段ガードの被覆ゲート (`parse.promote_absent_by_fresh_odds`, 2026-07-06 同日)**: 朝スキャン初日に2つ目の実障害 — **NAR の朝は無投票馬が単勝オッズ表に載らない**ため、「fresh 単勝に無い馬=取消」の二段ガード (2026-06-10, keibago/JRA/oddspark 3経路) が盛岡R2 で **11頭中10頭を absent に誤昇格** (1頭だけの幻レースを snapshot 保存、R3-R12 も 3-6割の馬が脱落した部分フィールドで勝負判定)。これまで顕在化しなかったのはスキャンが常にプールの立った午後だったため。→ 3経路を `parse.promote_absent_by_fresh_odds` に共通化し**二重ゲート**: ①**時間ゲート** = 発走45分前より手前では発動しない (被覆ゲートだけでは不十分 — 実機: 盛岡R2 朝は被覆 9/11=82% が閾値80%を超えて無投票2頭を誤昇格・DebaTable 取消マーク無し。公式取消は DebaTable/出馬表 absent フラグが時間帯によらず処理) ②**被覆ゲート** = fresh 単勝が非 absent 馬の8割以上を覆うときだけ (実際の取消は1-2頭=高被覆に収まる)。bet 帯 (締切1-2.5分前)・パドック再score (2-7分前) は両ゲートを自然に通過し従来どおり発動。誤保存 snapshot は次スキャンが全馬フィールドで上書き (自己修復)。`tests/test_scrape_keibago.py::test_promote_absent_coverage_gate`。
  - **付随修正2: DebaTable 空のリトライ (2026-07-06 同日)**: 3つ目の実障害 — 指数再利用の朝スキャンは LLM が走らず 36R のスクレイプが4並列・数十秒に凝縮され、**keiba.go.jp のレート制限で DebaTable だけ空**が返る (単発なら9頭取れるのをスキャン burst 中だけ失敗) → 単複馬リスト (=オッズが付いた馬のみ) に縮退し、朝の空プールと合成すると **9頭が3頭の幻フィールド**になる (盛岡R4 実機)。→ `analyze_keibago` が DebaTable 空のとき 3s/6s 待って最大2回再試行。従来スキャン (LLM あり) はスクレイプが自然に分散するので顕在化しなかった。**JRA 開催日 (週末) が自動で計測対象になる**のが主目的の一つ (JRA は pool が深く自票インパクトが無い = 確証が出たとき実弾をスケールできる唯一の区分なのに 9R しか無かった)。`tests/test_shobu_auto.py` (hour gate / mtime ガード / state 永続 / 無効化)。
- **勝負レース(推奨)の締切5-7分前 自動再score (`api.main.ShobuPaddockRescorer`, make api 常駐, ユーザ指示 2026-06-30)**: 「勝負レース取得後も make api を回しておけばパドック情報を取れるように」。**make api 単独ではパドックが入らない**問題 (shobu の Claude 指数は scan 時刻=締切のずっと前に生成され、make api の常駐ループは結果取得 (ResultAutoFetcher) と shobu の market-only refresh (--no-llm) のみで指数を再生成しない) に対し、**make api 内に1分毎の常駐ループ**を追加。当日 `data/cache/shobu/<date>.json` の **recommended な NAR/JRA レース** を見て、**締切 `MIN_LEAD_SEC`(2分)〜`WINDOW_SEC`(7分) 前** に入ったものを score 段 subprocess (`scrape_keibago`/`scrape_jra --phase=score --snapshot`、env `KEIBA_SCORE_TIMEOUT=200`/`QUERIES_PER_HORSE=4`) で **Claude 指数をパドック込みで再生成** → snapshot 上書き → 乖離(基準B)/市場一致シグナルも自動反映。実弾投票はしない・watch-auto 非依存 (snapshot 再生成は idempotent)。**再score 失敗/timeout は無害**: `analyze._run_score_stage` が scores 空なら `.llm.json` を上書きしないので scan 時点指数は消えない (最悪「指数据え置き+オッズ更新」)。window 内で `_fired` により1レース1回のみ撃つ (当日 reset)。lifespan で起動/停止、状態 `GET /api/shobu/paddock-rescore`。`tests/test_shobu_auto.py::test_paddock_rescorer_due_window`。
  - **発火イベントの永続化 + 効果は現状測定不能 (2026-07-06)**: 再score の発火記録が in-memory のみで効果検証できなかったため、`_rescore` が発火毎に **`data/cache/paddock_rescore_events.jsonl`** へ 1 行 append (rid/netkeiba/date/fired_at/rc/duration_sec/ok、書き込み失敗は呑む) — 今後の「再score 有無 × 戦略ROI」join キー。proxy 分析 (llm_scored_at→発走 lead time, 108R) の実測: **median 84分・締切間際 (≤10分) 採点は 11/108 のみ・no-op バグ修正 (07-04) 後は 0R** = パドック反映の効果はまだ何も言えない。再score 対象が recommended 限定なので near 窓サンプルの蓄積は遅い — 対象拡大はコスト増 (締切帯の claude -p 集中) とのトレードオフで宿題。
...
5. **Claude 考察 → 各馬指数 (2段パイプライン, 2026-05-31〜)**: LLM (`claude -p`) の役割を「最終買い目の picks/cuts 選定」から **「各馬の強さ指数 (0-100, 高い=1位) を出す」** へ変更し、その指数を確率モデルの fundamental に合成する。
   ...
   - **score ステージ** (締切5-7分前): `llm.score_horses` (dispatcher) が Tavily/WebFetch で各馬の適性・状態・取消を調べ `data/predictions/<race_id>.llm.json` に指数キャッシュ。`analyze._run_score_stage` が4経路 (netkeiba/JRA/keibago/oddspark) 共通で呼ぶ。
     ...
     - **固定クエリ Tavily プリフェッチ = ARCH-B (`KEIBA_SCORE_RESEARCH=prefetch`, 既定 OFF, 2026-07-05 ユーザ指示「毎回Claudeで検索するのは現実的でない。Claudeが使うと想定されるクエリを固定して自動でtavily検索できるか」)**: score 段のリサーチを「LLM が MCP で対話検索」(ARCH-A) から「**Python が固定テンプレのクエリを Tavily API 直叩き** (`src/research_prefetch.py`, stdlib urllib・MCP/claude 非依存) で dossier を作り、**claude -p は読解+採点の 1 セッションのみ**」に置き換える。根拠 = tool_usage ログ実測 (85R・検索1,286本): Claude のクエリは パドック/馬体重43%・馬名+場+クラス21%・馬場9%・予想7%・取消7% と **8割超がテンプレ的**で、LLM の本体価値は読解 (同名別馬/古記事の排除)・±調整・evidence 化にある → そこだけ採点段に残す (`build_horse_score_from_dossier_prompt` が「生スニペット=未検証、関連判定して仮指数±調整」を指示)。クエリ構成: レース級3本 (馬場/取消/予想 — レースで1回= ARCH-A のシャード重複が消える) + 各馬2本 (パドック・近況) + ユニーク騎手 (上限8)。dossier は `data/cache/research/<race_id>.json` に TTL 900s キャッシュ (再score/リトライで再フェッチしない)。**dossier 不可 (TAVILY_API_KEY 無し/全滅) は agentic に自動フォールバック**し、`.llm.json` の `research_mode` / snapshot の `llm_research_mode`・`llm_model` に **実績値** (dispatcher の `llm.LAST_RESEARCH_MODE`) を刻む = A/B 計測の分割キー (prefetch 発行クエリも tool_use `tavily_prefetch` → tool_usage kind="prefetch" で永続)。効果: claude セッション ~1/5・Tavily/Anthropic 輻輳の構造的回避。**失うもの**: 適応的フォローアップ + 結果ページの取捨選択 (実測の~2-3割) → 数十Rの A/B (research_mode 分割) で劣化を検証してから既定化を判断。env: `KEIBA_PREFETCH_RESULTS`(4)/`KEIBA_PREFETCH_DEPTH`(basic)/`KEIBA_PREFETCH_CONCURRENCY`(4)/`KEIBA_PREFETCH_TTL_SEC`(900)。配線: shobu CLI `--research prefetch` / `ShobuScanRequest.research` / Web UI 勝負レース設定「リサーチ方式」Select。watch-auto 等は env で opt-in。`tests/test_research_prefetch.py`。
     - **A/B 自動割当 `--research ab` = shobu の新既定 (2026-07-06, 回収率コンサル実行)**: prefetch 腕が opt-in のまま n=8 で A/B が永久に始まらない問題への処方。`shobu._ab_research_arm(rid)` が **md5(netkeiba_rid) 先頭バイトの偶奇**で "prefetch"/"agentic" を per-race 決定論割当 (組込み hash() は PYTHONHASHSEED 依存なので不使用。再実行/リトライ/別プロセスでも同 rid=同腕、集計側で腕を再計算できる ITT 設計)。優先順位: **env `KEIBA_SCORE_RESEARCH` 明示 > ab 割当** (operator 明示尊重の既存流儀)。`--research agentic/prefetch` 明示は従来どおり全レース同一。割当 env は `_pass_env(attempt)` の**後**に適用されリトライ軽量化と両立 (腕は維持)。prefetch 腕の dossier 不可→agentic 内部フォールバックは許容 (計測は snapshot の実績 `llm_research_mode` で分割するので汚染しない)。API/nightly/daily も既定 "ab" (`ShobuScanRequest.research`・`KEIBA_NIGHTLY_RESEARCH`・`KEIBA_DAILY_RESEARCH`、`build_shobu_cmd` は常に `--research` を明示で渡す)。**判定は `scripts/research_mode_ab.py`** (読み取り専用: mode 別 n・指数生成成功率・win1/place1/place2/quinella12 ROI・Claude#1 勝率。各腕 30-50R 貯まったら prefetch の劣化有無を判定 → 劣化なしなら prefetch 一本化でセッション ~1/5 = スループット 3-5x が解放される)。`tests/test_shobu.py` (決定論/50-50 分布/明示優先/リトライ腕維持/CLI 既定)。
   ...
   - **✅ 較正済 (2026-07-05, `scripts/sweep_llm_blend.py`)**: `T_LLM`/`LLM_BLEND_DEFAULT` を勝者 log-loss (proper scoring rule) の grid sweep で初較正 (n=131, 市場非依存 era・発走前採点・結果確定・fundamental は市場 implied 近似 = MLE α≈0 が根拠)。**旧値 (T=25, w=0.75) は市場単独より一貫して悪い** (log-loss 1.909 vs 1.810 = +0.10、前半/後半・v1/v2/v3 全分割で +0.08〜+0.12 と再現) — これは EV束 ROI 51.5% のメカニズム (w 大 → 最終Pが Claude の言い分で市場から乖離 → その乖離が勝率次元で無情報 → 乖離箇所だけ買って負ける adverse selection)。**新値 T_LLM=10 / LLM_BLEND_DEFAULT=0.25** (grid 最小セル 1.801。ただし市場単独比 -0.009 は分割で反転するノイズ域 = 確率合成での Claude 付加価値は現状ゼロが誠実な結論。w=0 も統計的同等だが、ユーザ決定 2026-06-13 の Claude 反映を残しつつ出血を止める妥協点)。**順位ベースの価値 (shobu 戦略くらべ・基準B・プレレジ台帳) はこれと独立で正の傾向** — 「順位には価値の兆候・確率の絶対値には無い」が現状実測。+100R 毎に再 sweep。旧記述「最終Pで Claude ~75%」の実効重み計算は旧値時代の記録として上記に残る。**注意**: 旧記述「指数 vs モデル / 既定 0.5 / fundamental 段で合成」は 2026-06-13 に廃止 (合成順を market→Claude に変更し既定 0.5→0.75)。EV束 (`recommended_bundle`) の probs も同じ `estimate_probs` 出力を使うため、Claude スコア済の馬では EV束も最終Pが Claude 主導 (~75%) になる (市場アンカーは未スコア馬・指数無しレースにのみ残る)。
...
- 新規 edge 候補の優先順: ①**odds_timeline の late-money momentum** — ✅ 一次検証済 (2026-06-12, `scripts/backtest_momentum.py`, n=85) → **✅ 再検証済 (2026-07-06, n=155 = NAR107/JRA27/ばんえい21・発見後 OOS `--since 20260613` n=53)**: 買い側は引き続き有意でない (mid5-15 短縮 116-126% だが most_shortened z は +0.41→−0.06 で消滅 = 買いシグナル化の根拠なし)。**ドリフト側 (r=bet/score>1.05) のカット候補は「NAR 限定」に条件が狭まった**: NAR ドリフト脚 flat 単勝 ROI 28.7% (n=585脚)・発見後 OOS でも 32.8% (n=224脚) vs 短縮脚 83-92% と両期間一貫。一方 **JRA ドリフト 95.9% (n=147脚)・mid5-15 ドリフト 106.8%・ばんえいドリフト 171.6%** で「全バンド最悪」(06-12) は不成立 — 効果は NAR×人気薄 (long>15 ドリフト 36-39%) に集中。→ **プレレジ (定義凍結 2026-07-06): 「NAR のみ・単勝 r=bet/score>1.05 の脚をカット」**。prospective 判定は `backtest_momentum.py --since 20260707` で蓄積後に再実行 (odds_timeline は自動で貯まり続ける・現蓄積 509 ファイル)。z 単独では未有意 (全期間 −1.46 / OOS −0.90) なので実弾接続はせず、カットは下振れ損失が小さい非対称介入として paper 追跡のみ ②JRA クロスプールのペーパー継続 ③(✅ 済) 日付 split 修正 + ばんえい分離 + 再学習。
