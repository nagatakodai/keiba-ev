...
- **ダッシュボードを 地方/中央 に分離 (2026-07-05 ユーザ指示「JRAはいったん別ページに避難して。ダッシュボード（地方）とダッシュボード（中央）で分ける」・backend/frontend の2エージェント並列実装)**: 計測系 4 エンドポイント (`/api/shobu/pnl`・`/api/shobu/indexed-pnl`・`/api/shobu/strategies-pnl`・`/api/shobu/indexed-strategies-pnl`) に **query param `venue=nar|jra`** を追加 (`api/store.py:_venue_filter`: jra=race_type=="jra" / **nar=それ以外でばんえい含む** / 省略=全件の後方互換・不正値 422・`version` と AND 併用可)。母集団 dict 段階で絞るので recommended_total/skipped_*/races_detail も venue スコープ。frontend は `web/components/DashboardView.tsx` (venue プロップ付き共有 server component) に本体を抽出し、**`/` = ダッシュボード（地方）(venue=nar)** / **`/jra` = ダッシュボード（中央）(venue=jra)**、ナビは「ダッシュボード(地方)/(中央)」の2リンク。**研究系カード (SignalRulesCard プレレジ台帳・MarketAgreementCard 買い方マトリクス) は venue を条件/軸として内包するグローバル研究台帳なのでフィルタせず地方ページのみに表示** (中央ページは fetch 自体しない)。実測分割: 全110R = 地方101R + 中央9R。**研究中シグナル表の緑色表示 (同日ユーザ指示)**: 発見時/登録後セルの ROI≥100% は数値%を緑 (text-emerald-300)、登録後が緑のルールは条件の定義テキストも緑 (broken の rose が優先)。`tests/test_shobu_pnl.py::test_venue_filter_splits_nar_and_jra`。
