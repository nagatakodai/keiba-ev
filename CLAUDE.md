...
- **当日キャッチアップ自動スキャン (`api.main.ShobuDailyCatchupScanner`, make api 常駐, 2026-07-06 回収率コンサル実行)**: 台帳蓄積が ~9R/日 (108R/15日・JRA はわずか 9R) しかなくプレレジ確証のボトルネックだった主因 = **当日スキャンが手動ボタン依存** (nightly は「翌日」を21時に1回だけ・make api が夜間落ちていれば当日データ無し)。対策として **毎日 `KEIBA_DAILY_SCAN_HOUR` (JST, 既定9時・6-15クランプ) 以降の最初の tick で当日スキャン (race_type=all・claude_all=True) を1回だけ自動起動**。起動条件: 当日 `data/cache/shobu/<today>.json` が**存在しない or mtime が当日 06:00 JST より前** (=前夜 nightly が作った翌日ファイルのまま → 当日オッズ付与+新規掲載場の取り込み価値あり。指数は再利用でほぼ無償)。当日 06:00 以降に手動等でスキャン済なら skip。実行済みガードは `data/cache/shobu/daily_scan_state.json` に永続 (reload 二重スキャン防止)。`KEIBA_DAILY_SCAN=0` で無効・research 既定 "ab" (`KEIBA_DAILY_RESEARCH`)。状態 `GET /api/shobu/daily`。**JRA 開催日 (週末) が自動で計測対象になる**のが主目的の一つ (JRA は pool が深く自票インパクトが無い = 確証が出たとき実弾をスケールできる唯一の区分なのに 9R しか無かった)。`tests/test_shobu_auto.py` (hour gate / mtime ガード / state 永続 / 無効化)。
