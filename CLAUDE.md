...
### bet-type-specific market_blend (Phase 19/20, holdout 291 races 由来)

`src/eval_holdout.py` の real-odds 評価で β の最適値が bet type ごとに違うことが判明。Plan ごとに β を使い分ける:

| Plan / bet type | β | 根拠 |
|---|---|---|
| Plan A/B/C/H2、単勝 EV table、3 連単 EV table | **0.78** (`BLEND_DEFAULT`) | 単勝 ROI peak 95.9% (市場 88.5% +7.4pt)、3 連単 PL hit rate β=0.7-0.8 plateau |
| Plan H1 (確率最優先 3 点) | **0.0** (`BLEND_HIT_PURE`) | holdout で唯一 +EV (ROI 109.6%, hit 15/291)。pure LightGBM が市場と独立した outsider 信号を出している |
| Plan G (適性ゲート → EV 足切り) | **1.0** (`BLEND_APTITUDE_GATE`) | holdout で +EV (ROI 108.1%, hit 5/291)。β=0→1 で単調 0→108% の clean signal。aptitude index 自体が model-like 信号を 8 因子合成しているため、市場暗黙率と PL 連鎖だけで +EV triple を発見できる (Phase 20) |

`src/analyze.py` は同じ `rd` に対して `estimate_probs` を 3 回呼び、`plan_rows` (default β=0.78) / `plan_rows_hit` (β=0) / `plan_rows_apt` (β=1.0) を作って各 Plan で使い分ける。Plan A/B/C/H2 は default、H1 は hit、G は apt。
