...
- **市場一致シグナルの自動蓄積 (2026-06-30 ユーザ指示「市場一致一本に絞って蓄積→確証を取れるよう自動化」)**: 結果取得ループ (`_run_once`) が `process_pending` の後に `append_market_agreement_history` を呼び、**Claude#1 が市場1番人気と一致するか (consensus)** で券種 ROI を分割した現在値 (`compute_market_agreement`: 市場非依存レースで agree/disagree に分け 馬連/組合せ系/3連複BOX/本命系 の ROI と差Δの bootstrap CI を出す) を `data/cache/market_agreement_history.jsonl` に時系列追記する (`races` 不変なら dedup=no-op)。**Δの 95%CI が 0 を跨がなくなれば確証** (`significant`)。初期所見 (72R): 3連複BOX は Δ−72pt CI[−154,−7]=**既に有意** (不一致=Claude contrarian 時に伸びる)、馬連は Δ+103pt だが CI[−28,+298]=要蓄積 (一致時に伸びる仮説)。`GET /api/shobu/market-agreement` (current+history)、ダッシュボードに `MarketAgreementCard` (券種別 一致/不一致 ROI・Δ・CI・★確証/蓄積中) を表示。background: 買い方ガイド (`web/lib/betGuide.ts`) は当面 Claude 単独自信度のみで、市場次元は CI が固まってから追加 (memory `feedback_strategy_confidence_rerun`)。`tests/test_shobu_pnl.py::test_market_agreement_splits_by_consensus`。
