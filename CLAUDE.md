...
- **累計露出の注意 (重要)**: per-race ハードリミット (¥20,000) は **1レース分の保証**であって `#buylist` 全体の総額ではない。**購入確定は #buylist 全体を一括購入する**ので、daemon が複数レースを溜めた状態で確定すると全部買う。→ **レースごとに確定/クリアする**のが基本。`BettingSession` は `_session_staked` (本セッション投入累計, 未確定含む) を表示し、per-race 上限を超えたら警告する (確定を検知できないので累計はリセットされない=溜め過ぎ検知用)。
