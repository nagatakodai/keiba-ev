...
**オッズパーク 半自動投票 (`src/oddspark_bet.py`, B案)**: claude 選定後の `recommended_bundle.legs` を**オッズパークのカートに投入するところまで** Playwright で自動化し、**購入確定は人が headful ブラウザで目視して押す** (誤発注の最終ゲート)。認証は**環境変数のみ** (`ODDSPARK_ID`/`ODDSPARK_PASSWORD`/`ODDSPARK_PIN`)、コード/ログ/コミットに残さない。合計賭金ハードリミット (既定¥10,000) でネット接続前に暴走を弾く。**認証後 (投票) 画面のセレクタ/URL は当方不可視のため `SELECTORS`・`_vote_url`・式別マッピングが best-effort placeholder** = ログインして DevTools で確認し**そこだけ直す** (各 step で `data/cache/oddspark_step_*.png` にスクショ)。`--manual-login` で認証情報を使わず人が手でログイン (最も安全)。オッズパーク利用規約は自動化を制限し得るので**半自動 (人が確定) に限定**。`python -m src.oddspark_bet <netkeiba_nar_rid>`。
