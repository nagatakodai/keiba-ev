...
**オッズパーク投票 (`src/oddspark_bet.py`)**: 既定は **半自動 (B案)** = claude 選定後の `recommended_bundle.legs` を**カートに投入するところまで** Playwright で自動化し、**購入確定は人が headful ブラウザで目視して押す**。`--auto-purchase` で**全自動 (実弾) モード** に切替え、#gotobuy → 投票申込確認画面 (`VoteConfirmOpcoin.do`) → **`#buy` (`<a id="buy" onclick="clickVoteComplete()">投票を申込</a>`)** → `VoteCompleteOpcoin.do` で確定 POST まで人の介入なしで実行する (実機 HTML 確認済 2026-05-28、`AUTO_PURCHASE_VERIFIED=True`)。安全策は四段構え: ①`max_total_stake` per-race (既定¥10,000) ②`daily_cap` 日次累計上限 (`--daily-cap=50000`、JST 日跨ぎで自動 reset、`data/cache/oddspark_daily_stake.json` に永続) ③`AUTO_PURCHASE_VERIFIED` 実機検証フラグ (True なら実弾可、緊急時に False に戻せば fail-safe で実弾停止) ④success marker (受付完了/投票完了 等のテキスト) を必ず検出してから daily_stake を加算 (誤検知/失敗で二重購入しない)。認証は**環境変数のみ** (`ODDSPARK_ID`/`ODDSPARK_PASSWORD`/`ODDSPARK_PIN`)、コード/ログ/コミットに残さない。`--manual-login` で認証情報を使わず人が手でログイン (最も安全)。オッズパーク利用規約による自動化制限のリスクは使用者が負う。`python -m src.oddspark_bet <netkeiba_nar_rid|race_id>` (netkeiba 12桁 rid・内部 race_id `<cup>-<si>-<rn>` どちらも可)。
...
**JRA 即PAT 投票 (`src/ipat_bet.py`)**: oddspark (NAR) の対になる **JRA 公式 IPAT 自動投票**。同じ設計 (env 認証 / SELECTORS 集約 / `BettingSession`+`run_session` daemon / queue / 四段安全ゲート / watch-auto 連携) を JRA 用に移植した scaffold。**JRA レース (venue code 01-10) のみ**を対象 (`_is_jra_rid`)、NAR は oddspark 経路へ。queue は `data/cache/ipat_bet_queue/` (oddspark とは別 namespace)、daily_stake は `data/cache/ipat_daily_stake.json` で独立管理。認証は**環境変数のみ**: `IPAT_INETID` / `IPAT_SUBSCRIBER` (加入者番号) / `IPAT_PARS` (P-ARS番号) / `IPAT_PIN` (暗証番号)。既定は半自動 (カート投入まで自動・**購入確定は人**)、`--auto-purchase` で全自動。`python -m src.ipat_bet <netkeiba_jra_rid|race_id>` (one-shot) / `--session` (daemon)。watch-auto は `--bet-ipat` で発走前 JRA の束を enqueue (`_enqueue_ipat_bet`)。Makefile: `make watch-auto BET_IPAT=1` または `make watch-auto-ipat-bet` (daemon+ループ 1 コマンド)。
- **⚠ 実機 DOM 未検証 (要調整)**: ログインの先 (投票画面) の HTML は当方から不可視なので、`SELECTORS` / URL / 式別コードは **best-effort placeholder**。oddspark が辿った道と同様、実機ログイン後に DevTools で確認して 1 箇所 (`SELECTORS`) を直す。各ステップで `data/cache/ipat_step_*.png` にスクショを残す。**`AUTO_PURCHASE_VERIFIED=False` (確認画面 DOM 未検証なので fail-safe で実弾を撃たない)** + **`_ORDERED_BETS_VERIFIED=False` (馬単/3連単は順序付き列の検証まで中止)**。実機で検証したらこの 2 フラグを True にする。IPAT は購入確定前に「合計金額/件数」の二重確認入力がある想定で、件数の自動算出は誤発注リスクのため未確定 (実機検証時に確定)。
- **IPAT 投票フロー (placeholder)**: ログイン (INET-ID → 加入者番号/P-ARS/暗証番号) → 通常投票 → 開催(場)選択 → R 選択 → 式別 → 馬番 (順不同=着順1列に各1頭=1組、順序付き=1/2/3着列) → 金額 (100円単位) → セット → 入力終了 → 購入予定リスト → (二重確認) → 購入する → 完了。oddspark の「馬番累積バグ」の教訓で各脚前に `_reset_umaban`。
