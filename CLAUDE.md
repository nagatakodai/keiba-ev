...
### bet-type-specific market_blend (Phase 19/20/21 + CV finding)

`src/eval_holdout.py` の real-odds 評価で β の最適値が bet type ごとに違うように見えたが、`scripts/cv_beta.py` の 5-fold CV で **Plan H1/H2 の β=0 は overfit** と判明 (mean hold-out ROI 64% < 77.5%)。Plan G の β=1.0 のみが CV robust (全 fold で β=1.0 best, std=0, mean hold-out ROI 106.6%)。

production 設定:

| Plan / bet type | β | CV evidence |
|---|---|---|
| Plan A/B/C/H1/H2、単勝 EV table、3 連単 EV table | **0.78** (`BLEND_DEFAULT`) | conservative default (CV で他を否定) |
| Plan G (適性ゲート → EV 足切り) | **1.0** (`BLEND_APTITUDE_GATE`) | **CV robust** (全 5 fold で β=1.0 が best, std=0) ★ |

`src/ev.py` には `BLEND_HIT_PURE = 0.0` 定数も残っている (実験用、production は使わない)。CLI から `--market-blend 0` で個別に試せる。

`src/analyze.py` は同じ `rd` に対して `estimate_probs` を 3 回呼ぶ:
- `plan_rows` (β=0.78): Plan A/B/C/H1/H2
- `plan_rows_apt` (β=1.0): Plan G のみ
- `plan_rows_hit` (β=0.0): 互換のため計算するが、production の Plan には使わない (実験 / 将来 sweep 用)

### Plan の実証的な階層 (5-fold CV mean hold-out ROI)

| Plan | 評価 | CV mean hold ROI | コメント |
|---|---|---|---|
| **G** | ★ robust +EV | 106.6% | 全 fold で β=1.0 が best (std=0)、唯一 CV で +EV を保つ |
| H1 | 不安定 | 64.7% | best β [0.05, 0.05, 0.05, 0.05, 0.50]、out-of-sample で 77.5% 割れ |
| H2 | 不安定 | 64.0% | best β [0, 0, 0, 0, 0.35]、同上 |
| A | noise | — | hit 0-2 で結論できず |
| B | -EV | — | 0/291 hit、楽観バイアスの罠 |
| C | noise | — | hit 2-3 で結論できず |

長期黒字を狙うなら **Plan G** が現状の証拠で唯一支持される。Plan H1/H2 は in-sample で +EV に見えても CV で再現せず、N=291 では結論できない。データ蓄積後に再 sweep を要する。Plan A/B/C は検索 MCP の補強根拠で慎重にフィルタすること。
