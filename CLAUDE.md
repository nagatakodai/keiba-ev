...
- **馬単/3連単 (順序付き) は当面 fail-safe で自動投入を見送る** (`_ORDERED_BETS_VERIFIED=False`): まとめ画面の 馬単[裏目]/3連単[マルチ] サブオプション (= 着順入替えの全順列化) を確実に OFF にできると実機 DOM で検証できていないため。オンだと 順列数×stake で過剰投入になる。順不同 (単複/馬連/ワイド/3連複) はこの sub-option が無く実機検証済なので常に投入する。**precondition の実装は完了済**: ①`_uncheck_ura_multi` がラベル '裏目'/'マルチ' の checkbox を OFF (ordered bet のセット前) ②`_assert_combo_delta` がセット後に 組数 が +1 だけ増えたか検証 (裏目/マルチ/馬番累積の過剰投入を捕捉、組数を読めない順序付きは安全側で中止)。残るは **flag を True にする前に実機で1度「裏目/マルチ OFF → #buylist 組数=1」を目視確認** するだけ (`_combo_count` の組数テキスト読取と `_uncheck_ura_multi` のラベル紐付けが実 DOM で効くかは未検証)。確認できたら `_ORDERED_BETS_VERIFIED=True` に。
...
- **enqueue**: `auto_watch --bet-oddspark` で、発走5分前 dispatch が `rc==0` かつ snapshot に**束(legs)が非空**なら `_enqueue_oddspark_bet` が `QUEUE_DIR/<netkeiba_rid>.req` を atomic 書き込み。**NAR (投票 joCode がある場) のみ・見送り(束空)は投入しない・未投入のみ** (JRA は oddspark 投票非対応なので skip)。
- 運用: 端末A `python -m src.oddspark_bet --session` (ログインして放置) / 端末B `make watch-auto BET_ODDSPARK=1` (または `python -m src.auto_watch --bet-oddspark ...`) → 発走前 NAR の束が常駐ブラウザのカートに積まれ続け、人が目視で確定。`BettingSession` は one-shot `fill_cart` とも共用 (one-shot は1レース→input待ち→close、daemon は queue ループ)。
...
- `recommended_bundle`: 「Claude 総合オススメ」= 全 bet type 横断の **joint (同時) Kelly 最適まとめ買い束** (`src/portfolio.py`)。レースの完全な top-3 結果分布 (全 ordered triple, Σp=1) 上で束全体の E[log(資金)] を最大化した成長率最適配分。独立 Kelly の単純和ではなく相関・排他性を考慮。+EV (P×O≥1.02) が無ければ legs 空 = 見送り。
  ...
  - **claude -p 選定** (`llm.select_bundle_stream`, `analyze._validate_and_update_bundle`): **web 検索なし** (全ツール無効)。モデルの全 +EV 候補 (全 bet type, P×O 降順) + joint Kelly 束 + 適性をすべて提示し、claude が**数値判断のみで最終「買い目」(picks=買う leg id) を選定**。picks の候補だけで joint Kelly を再計算 (トリガミ防止込み) して **recommended_bundle を置換** (= claude が最終決定)。`llm_review` (picks/summary/confidence/notes, mode="selection") を添える。選定が完了しない/error 時はモデル束を維持。`--no-llm` 時はモデルのみ。後方互換で picks 不在・cuts のみの出力も処理。**束はまずモデルのみで生成・保存→ claude が選定して置換**するので、選定前は frontend で「総合オススメ (モデル)」+「Claude 検証前」、選定済 (`llm_review.validated`) で「Claude 総合オススメ」+ magenta バッジに切替。
    - **発走5分前の再判定**: watch-auto は window=5 (既定) で**発走5〜5+tolerance分前に1回 dispatch** → その時点で全オッズを再取得し estimate_probs〜束〜claude 選定まで通す (= 最新オッズでの最終判定)。netkeiba/keiba.go.jp/JRA/oddspark いずれの経路も dispatch 時に fresh fetch する。
