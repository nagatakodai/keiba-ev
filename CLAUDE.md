...
- **馬単/3連単 (順序付き) は当面 fail-safe で自動投入を見送る** (`_ORDERED_BETS_VERIFIED=False`): まとめ画面の 馬単[裏目]/3連単[マルチ] サブオプション (= 着順入替えの全順列化) を確実に OFF にできると実機 DOM で検証できていないため。オンだと 順列数×stake で過剰投入になる。順不同 (単複/馬連/ワイド/3連複) はこの sub-option が無く実機検証済なので常に投入する。`_ORDERED_BETS_VERIFIED` を True にする前に裏目/マルチ checkbox を OFF にする処理を `_add_leg_to_cart` に追加し #buylist で組数=1 を確認すること。
