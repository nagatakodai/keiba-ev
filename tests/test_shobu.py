...
def test_scan_max_races_keeps_nearest_start(monkeypatch):
    """取得レース数 (max_races) は発走日時が近い (早い) 順に N 件を採用する (ユーザ指示 2026-06-30)。"""
    now = int(time.time())
    disc = [
        {"race_id": f"20263206010{n}", "url": "u", "start_at": now + 3600 * n,
         "venue": "佐賀", "race_no": n, "source": "oddspark"}
        for n in (1, 2, 3, 4)   # start_at が n に比例 (race_no 1 = 最も早い=近い発走)
    ]
    ...
    assert nos == [1, 2], f"発走が近い 2 件 (race_no 1,2) を採用するはず: {nos}"
